...
byte    Sprite::Flags( )          {
  return pgm_read_byte( SpriteData + SpriteFlags );
}
...
uint8_t AISprite::Speed( )        {
  return pgm_read_byte( SpriteData + SpriteSpeed );
}
byte    AISprite::Intelligence( ) {
  return pgm_read_byte( SpriteData + SpriteIntelligence );
}
...
int Map::MinXPixel() {
  return FirstRoom * RoomWidth * TileSize;
}
int Map::MaxXPixel() {
  return ( ( LastRoom + 1 ) * RoomWidth * TileSize ) - 1;
}
int Map::MaxYPixel() {
  return ( ( MapHeight + 1 ) * RoomHeight * TileSize ) - 1;
}
int Map::MinXTile() {
  return FirstRoom * RoomWidth;
}
int Map::MaxXTile() {
  return ( ( LastRoom + 1 ) * RoomWidth ) - 1;
}
int Map::MaxYTile() {
  return ( ( MapHeight + 1 ) * RoomHeight ) - 1;
}
...
void SquarioGame::ProcessButtons( ) {
  ...
  if ( ButtonState[ButtonLeft] )  {
    Player.vx--;
    if ( Player.vx < MaxSpeed * -1 ) Player.vx = MaxSpeed * -1;
  }
  if ( ButtonState[ButtonRight] ) {
    Player.vx++;
...
void SquarioGame::Cycle( ) {
  ...
  if ( Event == ETPlaying && Player.y > MapPixelHeight ) {
    Event = ETDeath;
    EventCounter = 25;
  }
...
void SquarioGame::AddMob( const unsigned char * DataPointer, int x, int y ) {
  ...
  for ( uint8_t a = 0; a < SpriteCap; a++ ) {
    if ( !Mobs[a].Active ) {
      Mobs[a].Activate( DataPointer, x, y );
      return;
    }
