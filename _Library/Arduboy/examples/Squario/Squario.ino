...
void loop() {
  ...
  while ( Game.Event ) { // 0 is Off
    ...
    if ( currTime > prevTime + FPSDELAY ) {
      ...
      byte input = display.buttonsState();
      if ( input & B_BUTTON )       {
        Game.ButtonPress( ButtonJump );
      }   else {
        Game.ButtonRelease( ButtonJump );
      }
      if ( input & A_BUTTON )       {
        Game.ButtonPress( ButtonRun );
      }    else {
        Game.ButtonRelease( ButtonRun );
      }
      if ( input & UP_BUTTON )      {
        Game.ButtonPress( ButtonUp );
      }     else {
        Game.ButtonRelease( ButtonUp );
      }
      if ( input & DOWN_BUTTON )    {
        Game.ButtonPress( ButtonDown );
      }   else {
        Game.ButtonRelease( ButtonDown );
      }
      if ( input & LEFT_BUTTON )    {
        Game.ButtonPress( ButtonLeft );
      }   else {
        Game.ButtonRelease( ButtonLeft );
      }
      if ( input & RIGHT_BUTTON )   {
        Game.ButtonPress( ButtonRight );
      }  else {
        Game.ButtonRelease( ButtonRight );
      }
      Game.Cycle();
